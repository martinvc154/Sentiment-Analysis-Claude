...
RATE_LIMIT_DELAY = 0.5  # Seconds between API calls in batch mode
...
def analyze_sentiment(
    ...
    try:
        ...
        if not api_key:
            return {
                'status': 'error',
                'error': 'ANTHROPIC_API_KEY not set. Please enter your API key in the sidebar.'
            }
...
def analyze_batch(
    ...
    """
    Analyze sentiment of multiple texts with rate limiting.
    ...
    for idx, text in enumerate(texts):
        ...
        if idx < len(texts) - 1 and rate_limit_delay > 0:
...
def create_sentiment_chart(df: pd.DataFrame) -> go.Figure:
    ...
    """
    # Handle empty dataframe or missing sentiment column
