...
# Page configuration
st.set_page_config(
    page_title="RoBert - Sentiment Analysis",
    page_icon="🤖",
    layout="wide",
...
MAX_BATCH_SIZE = 100     # Maximum texts in batch analysis
...
# Header
st.markdown('<div class="main-header">🤖 RoBert</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Sentiment Analysis powered by Anthropic\'s Claude AI</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    model = st.selectbox(
        ...
        help="Select the Claude model to use for analysis. Claude Sonnet 4 recommended for best results."
    )
...
with tab1:
    ...
    if analyze_button and text_input and api_key:
        ...
        if len(text_input) > MAX_TEXT_LENGTH:
            st.error(f"Text is too long ({len(text_input)} characters). Maximum allowed is {MAX_TEXT_LENGTH} characters.")
        elif len(text_input.strip()) < 3:
            st.warning("Text is too short. Please enter at least 3 characters.")
        else:
...
with tab2:
    ...
    if texts_to_analyze:
        # Validate texts
        valid_texts = [t for t in texts_to_analyze if len(t.strip()) >= 3 and len(t) <= MAX_TEXT_LENGTH]
        skipped_count = len(texts_to_analyze) - len(valid_texts)

        if skipped_count > 0:
            st.warning(f"{skipped_count} text(s) skipped (too short or too long)")

        if len(valid_texts) > MAX_BATCH_SIZE:
            st.error(f"Too many texts ({len(valid_texts)}). Maximum allowed is {MAX_BATCH_SIZE}. Please reduce the number of texts.")
        elif len(valid_texts) == 0:
            st.error("No valid texts to analyze. Texts must be between 3 and 10,000 characters.")
        else:
            st.info(f"Ready to analyze {len(valid_texts)} texts")
